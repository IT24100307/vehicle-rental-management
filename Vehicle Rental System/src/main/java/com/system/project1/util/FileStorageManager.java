...
public class FileStorageManager {
    ...
    private void saveAllEvents(List<Event> events) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(EVENTS_FILE))) {
            ...
            // Log success
            System.out.println("Successfully saved " + events.size() + " events to file: " + EVENTS_FILE);
        } catch (IOException e) {
            // Log the stack trace and specific error message
            System.err.println("Error saving events to file: " + EVENTS_FILE);
            ...
            if (!file.exists()) {
                System.err.println("File does not exist: " + EVENTS_FILE);
            } else if (!file.canWrite()) {
                System.err.println("File is not writable: " + EVENTS_FILE);
            } else {
                System.err.println("File exists and is writable, but still failed to write");
            }
        }
    }
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(DISCOUNTS_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 5) {
                    ...
                    // Handle staffId field
                    if (parts.length > 10 && !parts[10].isEmpty()) {
                        discount.setStaffId(Long.parseLong(parts[10]));
                    }
    ...
    private void saveAllDiscounts(List<Discount> discounts) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(DISCOUNTS_FILE))) {
            for (Discount discount : discounts) {
                writer.write(String.format("%d|%s|%s|%.2f|%s|%b|%s|%s|%s|%b|%s%n",
                        discount.getId(),
                        discount.getName(),
                        ...
                        discount.getStaffId() != null ? discount.getStaffId().toString() : ""));
            }
        } catch (IOException e) {
