...
public class VehicleInventory {
    ...
    public Vehicle[] getVehicles() {
        // Return only non-rented vehicles
        return filterVehiclesByType(0);
    }
    ...
    public Vehicle[] filterVehiclesByType(int typeChoice) {
        ...
        if (typeChoice == 0) {
            // Create a copy of non-null, non-rented vehicles
            int nonNullCount = 0;
            for (int i = 0; i < vehicleCount; i++) {
                if (vehicles[i] != null && !vehicles[i].isRented())
                    nonNullCount++;
            }
    ...
    // Get all available (not rented) vehicles
    public Vehicle[] getAvailableVehicles() {
        ...
        Vehicle[] available = new Vehicle[availableCount];
        int index = 0;
        for (int i = 0; i < vehicleCount; i++) {
            if (vehicles[i] != null && !vehicles[i].isRented()) {
                available[index++] = vehicles[i];
            }
        }
    ...
    public Vehicle[] getRentedVehicles() {
        ...
        Vehicle[] rented = new Vehicle[rentedCount];
        int index = 0;
        for (int i = 0; i < vehicleCount; i++) {
            if (vehicles[i] != null && vehicles[i].isRented()) {
                rented[index++] = vehicles[i];
            }
        }
