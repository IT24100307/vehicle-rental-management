...
async function loadVehicles() {
    try {
        ...
        vehicles = response.filter(vehicle => !vehicle.rented);
...
function renderVehicles(vehicleList) {
    ...
    // Filter out any rented vehicles from display
    const availableVehicles = vehicleList.filter(vehicle => !vehicle.rented);
...
async function loadRentedVehicles() {
    ...
    try {
        ...
        response.forEach(rental => {
            ...
            const isRented = rental.rented !== false; // Check if vehicle is currently rented
            
            // Add a status indicator class
            if (isRented) {
                node.classList.add('currently-rented');
            }
            
            node.innerHTML = `
                <div class="node-title">
                    ${vehicle.brand} ${vehicle.model}
                    <span class="rental-status ${isRented ? 'active' : 'returned'}">${isRented ? 'Active' : 'Returned'}</span>
                </div>
                <div class="node-info">
        ...
        if (!selectedVehicle.vehicleID) {
          ...
          console.log('Attempting to rent vehicle:', {
            ...
            if (data && data.purchaseId) {
                // Mark the vehicle as rented locally before redirecting
                selectedVehicle.rented = true;
                
                // Update the vehicles array
                const vehicleIndex = vehicles.findIndex(v => v.vehicleID === selectedVehicle.vehicleID);
                if (vehicleIndex !== -1) {
                    vehicles[vehicleIndex].rented = true;
                }
                
                // Show a notification that the vehicle is being removed from inventory
                const notificationDiv = document.createElement('div');
                notificationDiv.className = 'vehicle-rented-notification';
                notificationDiv.innerHTML = `
                    <div class="notification-content">
                        <i class="fas fa-check-circle"></i>
                        <p>Vehicle ${selectedVehicle.brand} ${selectedVehicle.model} has been rented and removed from inventory.</p>
                        <p>Redirecting to payment page...</p>
                    </div>
                `;
                document.body.appendChild(notificationDiv);
                
                // Briefly display the notification before redirecting
                setTimeout(() => {
